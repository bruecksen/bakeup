...
class CustomerOrderForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if (
            self.production_day_product.production_plan
            and self.production_day_product.production_plan.is_locked
        ):
            self.fields["quantity"].disabled = True
        self.fields["quantity"].widget.attrs.update(
            {
                "min": 0,
                "max": self.production_day_product.calculate_max_quantity(
                    self.customer
                ),
            }
        )
...
class CustomerOrderBatchForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        if (
            production_day_product.production_plan
            and production_day_product.production_plan.is_locked
        ):
            self.fields["product"].disabled = True
            self.fields["quantity"].disabled = True
        self.fields["quantity"].widget.attrs.update(
            {"min": 0, "max": production_day_product.calculate_max_quantity()}
        )
...
class CustomerProductionDayOrderForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for production_day_product in self.production_day_products:
            if (
                not production_day_product.is_locked
                and not production_day_product.is_sold_out
            ):
                self.fields[
                    f"production_day_{production_day_product.product.pk}-product"
                ] = forms.IntegerField(widget=forms.HiddenInput, required=False)
                self.fields[
                    f"production_day_{production_day_product.product.pk}-quantity"
                ] = forms.IntegerField(label=_("Quantity"), required=False)
        ...
        for production_day_product in self.production_day_products:
            if cleaned_data.get(
                ...
                if quantity > production_day_product.calculate_max_quantity(
                    ...
                    raise forms.ValidationError(
                        _("Sorry, but we don't have enough products.")
                    )
        ...
        if self.forms:
            ...
            for product in production_day.production_day_products.all():
                products_qty["product_{}".format(product.product.pk)] = {
                    ...
                    "ordered_qty": 0,
                    "is_locked": (
                        product.production_plan and product.production_plan.is_locked
                    ),
                }
            for form in self.forms:
                for product in products_qty.keys():
                    ...
                    if ordered_qty:
                        products_qty[product]["ordered_qty"] = (
                            products_qty[product]["ordered_qty"] + ordered_qty
                        )
            if any(
                v["is_locked"] and v["ordered_qty"] > v["max_qty"]
                for k, v in products_qty.items()
            ):
                raise ValidationError(
                    _("There are more products ordered then max qty.")
                )
    ...
    def __init__(self, *args, **kwargs):
        ...
        for product in self.production_day.production_day_products.all():
            ...
            self.fields[field_name] = forms.IntegerField(
                required=False,
                label=_("{} (max. {})").format(
                    product.product.name, product.max_quantity
                ),
                widget=forms.NumberInput(attrs={"placeholder": _("Quantity")}),
            )
...
ProductionDayProductFormSet = modelformset_factory(
    ProductionDayProduct,
    fields=("product", "max_quantity", "is_published", "group"),
    extra=1,
    can_delete=True,
    formset=BaseProductionDayProductFormSet,
)
...
BatchCustomerOrderFormSet = formset_factory(
    form=BatchCustomerOrderForm,
    extra=0,
    formset=BaseBatchCustomerOrderFormFormSet,
)
